infixExpression = input("请输入中缀表达式：\n")


def evaluator(infixExpression):
    result = []  # 结果列表
    stack = []  # 栈
...
def getValue(suffixExpression):
    ret = []
    for i in suffixExpression:
        if i.isnumeric():
            ret.append(i)
        else:
            y = int(ret.pop())
...
suffixExpression = evaluator(infixExpression)
print("运算结果为:", getValue(suffixExpression))
